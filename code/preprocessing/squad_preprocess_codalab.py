...
def read_write_dataset(dataset, tier, prefix):
    ...
    qn, an = 0, 0
    skipped = 0
    context_file, question_file, quid_file, text_file, span_file = [],[],[],[],[]
            ...
            for qid in range(len(qas)):
                ...
                for ans_id in num_answers:
                    ...
                    try:
                        ...
                        quid_file.append(' '.join([qas[qid]['id']]) + '\n')
                        context_file.append(' '.join(context_tokens) + '\n')
                        question_file.append(' '.join(question_tokens) + '\n')
                        text_file.append(' '.join(text_tokens) + '\n')
                        span_file.append(' '.join([str(a_start_idx), str(a_end_idx)]) + '\n')
    ...
    return qn,an, quid_file, context_file, question_file, text_file, span_file
