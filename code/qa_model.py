...
class QASystem(object):
    ...
    def setup_system4(self):
        ...
        # output_p_fw = [B,P,h]
        # output_q_fw = [B,Q,h]
        output_p_fw = tf.expand_dims(output_p_fw, 2) # [B,P,1,h]
        output_q_fw = tf.expand_dims(output_q_fw, 2) # [B,Q,1,h]
        Wq = tf.get_variable("Wq", shape=[1, 1, h_dim, h_dim], initializer=tf.contrib.layers.xavier_initializer())
        Wp = tf.get_variable("Wp", shape=[1, 1, h_dim, h_dim], initializer=tf.contrib.layers.xavier_initializer())
        Wr = tf.get_variable("Wr", shape=[1, 1, h_dim, h_dim], initializer=tf.contrib.layers.xavier_initializer())
        bp = tf.Variable(tf.zeros([1,1,h_dim]))
        b = tf.Variable(tf.zeros(()))
        w = tf.get_variable("w", shape=[1, 1, h_dim], initializer=tf.contrib.layers.xavier_initializer())

        hr_first = tf.zeros([cur_batch_size,1,1,h_dim])

        lstm = tf.contrib.rnn.BasicLSTMCell(h_dim)
        lstm = tf.contrib.rnn.DropoutWrapper(cell=lstm, output_keep_prob=self.dropout)
        # forward dir
        hrf = [None] * P
        state = lstm.zero_state(cur_batch_size, tf.float32)
        for i in range(P):
            with tf.variable_scope("pre_forward", reuse=(i>0)):
                prev_hr = hr_first
                if (i != 0):
                    prev_hr = hrf[i-1]
                t1 = tf.reduce_sum(tf.multiply(Wq, output_q_fw), axis=3) #[B,Q,h]
                hp = output_p_fw[:,i,:,:]
                hp = tf.expand_dims(hp, 1) # [B,1,1,h]
                t2 = tf.reduce_sum(tf.multiply(Wp, hp), axis=3) #[B,1,h]
                t3 = tf.reduce_sum(tf.multiply(Wr, prev_hr), axis=3) # [B,1,h]
                t4 = t2 + t3 + tf.multiply(tf.ones([cur_batch_size,1,1]), bp) # [B,1,h]
                t4 = tf.multiply(t4, tf.ones([1,Q,1])) # [B,Q,h]
                t5 = t1 + t4 # [B,Q,h]
                Gi = tf.tanh(t5) # [B,Q,h]
                t6 = tf.reduce_sum(tf.multiply(Gi, w), axis=2) # [B,Q]
                t7 = t6 + tf.multiply(b, tf.ones([cur_batch_size,Q])) # [B,Q]
                ai = tf.nn.softmax(t7) # [B,Q]
                ai = tf.expand_dims(ai, 2)
                ai = tf.expand_dims(ai, 2)
                t8 = tf.multiply(output_q_fw, ai) # [B,Q,1,h]
                t8 = tf.reduce_sum(t8, axis=1) # [B,1,h]
                t8 = tf.squeeze(t8, axis=1) # [B,h]
                hp = tf.squeeze(hp, axis=1)
                hp = tf.squeeze(hp, axis=1) # [B,h]
                zi = tf.concat([hp, t8], 1) # [B,2h]
                output, state = lstm(zi, state)
                output = tf.expand_dims(output, 1)
                output = tf.expand_dims(output, 1)
                hrf[i] = output # [B,1,1,h]

        # backward dir
        hrb = [None] * P
        stateb = lstm.zero_state(cur_batch_size, tf.float32)
        for i in range(P-1,-1,-1):
            with tf.variable_scope("pre_backward", reuse=(i<(P-1))):
                prev_hr = hr_first
                if (i != P-1):
                    prev_hr = hrb[i+1]
                t1 = tf.reduce_sum(tf.multiply(Wq, output_q_fw), axis=3) #[B,Q,h]
                ...
                hp = tf.squeeze(hp, axis=1) # [B,h]
                zi = tf.concat([hp, t8], 1) # [B,2h]
                output, stateb = lstm(zi, stateb)
                output = tf.expand_dims(output, 1)
                output = tf.expand_dims(output, 1)
                hrb[i] = output # [B,1,1,h]

        hrf = tf.stack(hrf, axis=1) # [B,P,1,1,h]
        hrf = tf.squeeze(hrf, axis=2)
        hrf = tf.squeeze(hrf, axis=2)
        hrb = tf.stack(hrb, axis=1) # [B,P,1,1,h]
        hrb = tf.squeeze(hrb, axis=2)
        hrb = tf.squeeze(hrb, axis=2) # [B,P,h]
        hr = tf.concat([hrf, hrb], 2) # [B,P,2h]
        hr = tf.expand_dims(hr, 2) # [B,P,1,2h]
        # Final layer
        V = tf.get_variable("V", shape=[1, 1, h_dim, 2 * h_dim], initializer=tf.contrib.layers.xavier_initializer())
        Wa = tf.get_variable("Wa", shape=[1, 1, h_dim, h_dim], initializer=tf.contrib.layers.xavier_initializer())
        v = tf.get_variable("v", shape=[1, 1, h_dim], initializer=tf.contrib.layers.xavier_initializer())
        ba = tf.Variable(tf.zeros([1,1,h_dim]))
        c = tf.Variable(tf.zeros(()))
        ha = [None] * 2
        F = [None] * 2
        beta_before_softmax = [None] * 2
        beta_after_softmax = [None] * 2
        ha_first = tf.zeros([cur_batch_size,1,1,h_dim])
        statec = lstm.zero_state(cur_batch_size, tf.float32)
        for i in range(2):
            with tf.variable_scope("final_layer", reuse=(i>0)):
                prev_ha = ha_first
                if (i != 0):
                    prev_ha = ha[i-1]

                t1 = tf.reduce_sum(tf.multiply(V, hr), axis=3) #[B,P,h]
                t3 = tf.reduce_sum(tf.multiply(Wa, prev_ha), axis=3) # [B,1,h]
                t4 = t3 + tf.multiply(tf.ones([cur_batch_size,1,1]), ba) # [B,1,h]
                t4 = tf.multiply(t4, tf.ones([1,P,1])) # [B,P,h]
                t5 = t1 + t4 # [B,P,h]
                F[i] = tf.tanh(t5) # [B,P,h]
                t6 = tf.reduce_sum(tf.multiply(F[i], v), axis=2) # [B,P]
                t7 = t6 + tf.multiply(c, tf.ones([cur_batch_size,P])) # [B,P]
                beta_before_softmax[i] = t7 # [B,P]
                t9 = tf.nn.softmax(beta_before_softmax[i]) # [B,P]
                beta_after_softmax[i] = t9
                t9 = tf.expand_dims(t9, 2)
                t9 = tf.expand_dims(t9, 2) # [B,P,1,1]
                t8 = tf.multiply(hr, t9) # [B,P,1,2h]
                t8 = tf.reduce_sum(t8, axis=1) # [B,1,2h]
                t8 = tf.squeeze(t8, axis=1) # [B,2h]
                output, statec = lstm(t8, statec)
                output = tf.expand_dims(output, 1)
                output = tf.expand_dims(output, 1)
                ha[i] = output # [B,1,1,h]

        self.logits_start = beta_before_softmax[0]
        self.yp_start = beta_after_softmax[0]

        self.logits_end = beta_before_softmax[1]
        self.yp_end = beta_after_softmax[1]
